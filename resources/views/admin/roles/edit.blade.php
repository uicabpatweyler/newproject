...
@section('content')
  ...
  <div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-30">
    <div>
      ...
      <h4 class="mg-b-0 tx-spacing--1">Editar Rol</h4>
    </div>
  ...
  <div class="row row-xs">
    <div class="col-lg-12 col-xl-12 mg-t-0">
      <form method="POST" id="formEdit" name="formEdit" action="{{route('roles.update', $role->id)}}">
        @csrf
        @method('PATCH')
            ...
            <div class="row row-sm ">
              <div class="form-group col-sm-6">
                ...
                <input class="form-control @error('display_name') is-invalid @enderror"
                       ...
                       type="text"
                       value="{{old('display_name',$role->display_name)}}" minlength="4" maxlength="30" required>
              </div><!-- col -->
              <div class="form-group col-sm-6 d-flex flex-column align-items-start">
                ...
                <input class="form-control @error('description') is-invalid @enderror"
                       ...
                       type="text"
                       value="{{old('description',$role->description)}}" minlength="4" required>
              </div><!-- col -->
            </div>
                ...
                <div class="card-header d-sm-flex align-items-start justify-content-between">
                  ...
                  <div class="custom-control custom-switch">
                    <input type="checkbox" class="custom-control-input module"
                           name="permissions[]"
                           id="module{{$module->id}}"
                           value="{{$module->permission}}" {{$habilities->contains('name',$module->permission) ? 'checked' : ''}}>
                        ...
                        <tr>
                            ...
                            <div class="d-sm-flex align-items-start justify-content-between mr-2">
                              ...
                              <div class="custom-control custom-switch">
                                <input type="checkbox" class="custom-control-input module-option"
                                       ...
                                       value="{{$option->opt_resource}}" {{$habilities->contains('name',$option->opt_resource) ? 'checked' : ''}}>
                          ...
                          <td>
                            @foreach($option->permissions as $permission)
                              <div class="custom-control custom-switch">
                                <input type="checkbox" class="custom-control-input the-permission"
                                       name="permissions[]"
                                       id="module{{$module->id}}-permission-{{$permission->name}}"
                                       value="{{$permission->name}}" {{$habilities->contains('name',$permission->name) ? 'checked' : ''}}>
...
<script>
    $().ready( function () {
      ...
      $('#formEdit').parsley({
        ...
        submitForm("POST","{{ route('roles.update', $role->id) }}", $("#formEdit").serialize());
      });
